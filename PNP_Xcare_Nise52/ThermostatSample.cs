...
namespace Thermostat
{
    ...
    public class ThermostatSample
    {
        ...
        private async Task TargetGPIOUpdateCallbackAsync(TwinCollection desiredProperties, object userContext)
        {
            ...
            (bool targetTempUpdateReceived, int targetGPIO) = GetPropertyFromTwin<int>(desiredProperties, propertyName);
            if (targetTempUpdateReceived)
            {
                _logger.LogDebug($"Property: Received - {{ \"{propertyName}\": {targetGPIO}°C }}.");

                string jsonPropertyPending = $"{{ \"{propertyName}\": {{ \"value\": {_GPIOValue}, \"ac\": {(int)StatusCode.InProgress}, \"av\": {desiredProperties.Version} }} }}";
                ...
                _logger.LogDebug($"Property: Update - {{\"{propertyName}\": {targetGPIO} }} is {StatusCode.InProgress}.");
                _GPIOValue = targetGPIO;
                string jsonProperty = $"{{ \"{propertyName}\": {{ \"value\": {_GPIOValue}, \"ac\": {(int)StatusCode.Completed}, \"av\": {desiredProperties.Version}, \"ad\": \"Successfully updated target temperature\" }} }}";
                var reportedProperty = new TwinCollection(jsonProperty);
                await _deviceClient.UpdateReportedPropertiesAsync(reportedProperty);
                _logger.LogDebug($"Property: Update - {{\"{propertyName}\": {_GPIOValue} }} is {StatusCode.Completed}.");                
            }
            else
            {
...
*/
        ...
        private async Task UpdatePropertyUpdate()
        {
            ...
            reportedProperties[propertyName_modelname] = "Nise52";
            reportedProperties[propertyName_GPIO] = _GPIOValue;
            reportedProperties[propertyName_workingSet] = 0xffff;
